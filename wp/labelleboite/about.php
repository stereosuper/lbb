			...
			<section id='equipe' class='section'>
				<div class='section-cell'>
					...
					<div class='container'>
						...
						<?php $loop = new WP_Query(array('post_type' => 'rat', 'posts_per_page' => -1));
						if($loop->have_posts()){ ?>
						...
						<?php if(have_rows('intervenants')){ ?>
							<ul class='presta-list small-presta-list'>
									...
									--><li class='presta-item'>
										<div class='square-link'>
											<?php if(get_sub_field('image')){ ?>
												<img src='<?php echo wp_get_attachment_image_src(get_sub_field('image'), 'presta-thumb')[0]; ?>' width='360' height='360' alt='<?php echo get_post_meta(get_sub_field('image'))['_wp_attachment_image_alt']['0']; ?>'>
											<?php }else{ ?>
												<img src='<?php echo get_template_directory_uri(); ?>/layoutImg/nobody.png' width='360' height='360' alt=''>
											<?php } ?>
											<span class='hover'>
												<span>
													<strong class='title'><span><?php the_sub_field('nom'); ?></span></strong>
													<span class='cat'><?php the_sub_field('poste'); ?></span>
								...
								--><?php } wp_reset_query(); ?><li>
									...
									<a href='<?php the_field('lienContact', 'options'); ?>' class='contact-link peignes'>
										<span>
											<i><?php the_field('intervenantsContact'); ?></i>
