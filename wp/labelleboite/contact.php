...
if(isset($_POST['submit'])){
    ...
    if(!$error){
        ...
        $subject = 'Nouveau message provenant de labelleboite.fr';
                ...
                <form action='<?php the_permalink(); ?>#contact' method='post' class='contactform <?php if($error) echo "formerror"; ?>' id='contact'>
                    <?php if(!$success){ ?>
                        ...
                        <fieldset <?php if($erreurMsg) echo "class='error'"; ?>>
                            ...
                            <?php $placeholderMsg = $devis ? 'Je suis intéressé(e) par la prestation "'.get_the_title($devis).'"' : ''; ?>
                            <textarea name='message' id='message' required placeholder='<?php the_field('placeholderMessage'); ?>' <?php if($devis || ($error && !$erreurMsg)) echo "class='on'"; ?>><?php echo $error ? $msg : $placeholderMsg; ?></textarea>
