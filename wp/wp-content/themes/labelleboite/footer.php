	...
	<?php
		function generateBullshitJs($btn, $img){
			if($btn){
				if($img){
					echo "['".$btn."', 'click', '".$img."']";
				}else{
		...
		?>
			...
			<?php foreach($bullshits as $bullshit) : ?>
				<?php switch($bullshit['bullshitContainer']){
					case 'purple': ?>
						purpleShits.push(<?php generateBullshitJs($bullshit['btnBullshit'], $bullshit['bullshitImg']); ?>);
						<?php break;
					case 'pink': ?>
						pinkShits.push(<?php generateBullshitJs($bullshit['btnBullshit'], $bullshit['bullshitImg']); ?>);
						<?php break;
					case 'blue': ?>
						blueShits.push(<?php generateBullshitJs($bullshit['btnBullshit'], $bullshit['bullshitImg']); ?>);
						<?php break;
					case 'yellow': ?>
						yellowShits.push(<?php generateBullshitJs($bullshit['btnBullshit'], $bullshit['bullshitImg']); ?>);
						<?php break;
					case 'white': ?>
						whiteShits.push(<?php generateBullshitJs($bullshit['btnBullshit'], $bullshit['bullshitImg']); ?>);
						<?php break;
				} ?>
			<?php endforeach; ?>
		<?php endif; ?>
		...
		function preloader(){
		    if(document.images){
		      	...
		      	if(hasClass(body, 'home')){
		        	...
		        	imgVous3.src = '<?php echo get_template_directory_uri(); ?>/layoutImg/vous/tronconneuse.png';
		      	}
