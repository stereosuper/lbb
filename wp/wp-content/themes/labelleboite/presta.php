		...
		<?php if ( have_posts() ) : the_post(); ?>
				...
				<div class='section-cell'>
					<div class='container'>
						...
						<h1 class='h2 light'>
							...
							<a href='#interventions' class='animTxt scrollTo'><?php the_field('title6'); ?></a><span class='animTxt'><?php the_field('title7'); ?></span>
                            <a href='#sensibilisation' class='animTxt scrollTo'><?php the_field('title8'); ?></a>.
						</h1>
			...
			<section id='ateliers' class='section'>
				<div class='section-cell'>
					...
					<div class='container'>
						...
						<?php } ?>
						<ul class='presta-list presta-list-colors1 small-presta-list'>
							<?php
            ...
            <section id='sensibilisation' class='section'>
				<div class='section-cell'>
					...
					<div class='container'>
						<?php $sensiTerm = get_field('sensiCat');
						$terms = get_terms('prestation-categorie', array('parent' => $sensiTerm));
						if($terms){ ?>
							<div class='presta-filtres'>
								<span>Les thèmes abordés:</span>
								<ul>
						...
						<ul class='presta-list presta-list-colors2 small-presta-list'>
							<?php
							$sensiTermSlug = get_term($sensiTerm, 'prestation-categorie')->slug;
							$loop = new WP_Query(array('post_type' => 'prestation', 'posts_per_page' => -1, 'tax_query' => array(array('taxonomy' => 'prestation-categorie', 'field' => 'slug', 'terms' => $sensiTermSlug))));
							if($loop->have_posts()){
								while($loop->have_posts()){ $loop->the_post(); $terms = wp_get_post_terms($post->ID, 'prestation-categorie'); ?><!--
									--><li class='presta-item' <?php if($terms){ echo "data-cat='"; foreach($terms as $term){ if($term->parent !== 0) echo $term->slug.','; } echo "'"; } ?>>
										<a href='<?php the_permalink(); ?>' class='square-link'>
