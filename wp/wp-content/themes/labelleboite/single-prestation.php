<?php get_header(); ?>
	...
	<?php if( have_posts() ){ ?>
		...
		<nav role='navigation' class='second-menu nav-single' id='fixedMenu'>
			<ul>
				<?php if($prev){ ?>
					<li class='left'><a href='<?php echo get_permalink( $prev->ID ); ?>' title='<?php echo $prev->post_title; ?>' class='h5'>La fiche précédente</a></li>
				<?php } if($next){ ?>
					<li class='right'><a href='<?php echo get_permalink( $next->ID ); ?>' title='<?php echo $next->post_title; ?>' class='h5'>La fiche suivante</a></li>
				<?php } ?>
			</ul>
		</nav>
		<?php } ?>
				...
				<div class='section-cell'>
					<div class='container'>
						...
						<strong class='sup-title'>
							...
							<?php if(get_field('duree')) echo get_field('duree'); if(get_field('participants')) echo ' | ' . get_field('participants'); ?>
						</strong>
